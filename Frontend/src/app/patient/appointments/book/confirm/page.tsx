...
const DoctorBookingPage = () => {
  ...
  const availableDays: number[] = doctor.availability.map((slot: { day: keyof typeof dayMap; time_slots: string[] }) => dayMap[slot.day]);
  ...
  const handleSubmit = () => {
    ...
    const bookingData = {
      ...
      status: "Pending",
      // end_time: "15:00", // This should be dynamically set based on the selected time slot
      booking_fee: doctor.consultation_fee,
      payment_status: "Paid",
      time_slot: appointmentDetails.start_time, // Send time_slot instead of start_time
    };

    const token = Cookies.get("token");
    axios
      .post(`${baseURL}/api/users/patient/appointments`, bookingData, {
