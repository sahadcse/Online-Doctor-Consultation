...
const TodaysConsultations = () => {
    ...
    useEffect(() => {
        const fetchConsultations = async () => {
            try {
                const response = await fetch(`${BACKEND_API_URL}/api/users/doctor/consultations`, {
                    headers: authHeader(),
                });
                if (!response.ok) {
    ...
    const handleApprove = async () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await fetch(`${BACKEND_API_URL}/api/users/doctor/consultations/${selectedAppointment._id}/confirm`, {
                        method: "PUT",
                        headers: {
                            ...authHeader(),
    ...
    const handleCancel = async () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await fetch(`${BACKEND_API_URL}/api/users/doctor/consultations/${selectedAppointment._id}/cancel`, {
                        method: "PUT",
                        headers: {
                            ...authHeader(),
                            "Content-Type": "application/json",
                                    ...
                                    {consultations.map((consultation) => (
                                        <tr key={consultation._id} className="hover:bg-gray-50 text-center">
                                            ...
                                            <td className="px-4 py-2 border">{
                                                new Date(consultation.consultation_date).toLocaleDateString()}</td>
                                            <td className="px-4 py-2 border">
                                                <span
                                                    className={`px-2 py-1 rounded-full text-sm ${consultation.status === "Waiting"
