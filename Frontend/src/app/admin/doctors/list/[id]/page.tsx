...
const DoctorDetails = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${baseURL}/api/admins/admin/doctors/${id}`, {
        headers: {
  ...
  const handleBlockUnblock = () => {
    const token = Cookies.get("token");
    const action = doctor?.status === "blocked" ? "unblock" : "block";
    axios
      .put(
        `${baseURL}/api/admins/admin/doctors/${doctor?._id}/${action}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      .catch((error) => {
        console.error(`There was an error ${action}ing the doctor!`, error);
      });
  };
